...
| 02 | [Drive folder → vector store, kept in sync](./02-drive-to-vector-store-upsert) | RAG ingestion where an edited file replaces its own vectors and a deleted file loses them, plus the chat assistant that queries it | ✅ | [✅](./02-drive-to-vector-store-upsert) |
